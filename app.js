...
function normalizeProjectsData(projectsList) {
  projectsList.forEach(p => {
    ...
    p.chapters.forEach(ch => {
      ...
      if (typeof ch.scratchpad !== 'string') ch.scratchpad = '';
...
function openWriterStudio(chapterId) {
  ...
  document.getElementById('writer-scratchpad-input').value = ch.scratchpad || '';
...
function onWriterMetaChange() {
  ...
  ch.scratchpad = document.getElementById('writer-scratchpad-input').value;
...
function sendScratchpadToBoard() {
  const text = (document.getElementById('writer-scratchpad-input').value || '').trim();
  if (!text) {
    alert("Escreva alguma ideia no bloco antes de fixá-la na mesa.");
    return;
  }
  ...
  const board = getCurrentBoard();
  const ch = proj.chapters.find(c => c.id === activeWritingChapterId);
  const chIdx = ch ? proj.chapters.findIndex(c => c.id === c.id) + 1 : 1;
  const chTitle = ch && ch.title ? ch.title : `Capítulo ${chIdx}`;

  const newItem = {
    id: Date.now(),
    type: 'note',
    color: 'yellow',
    title: `Ideia • Cap. ${chIdx}`,
    text: `[${chTitle}]\n${text}`,
    characterId: ch ? ch.povCharacterId : null,
  ...
  const btn = document.getElementById('btn-send-scratchpad');
  if (btn) {
    const originalText = btn.textContent;
    btn.textContent = "✅ Fixado na Mesa!";
    btn.classList.add('btn-primary');
    setTimeout(() => {
...
function saveChapter(e) {
  ...
  if (id) {
    ...
    proj.chapters.push({ id: Date.now(), content: '', scratchpad: '', wordCount: 0, ...chData });
  }
...
function exportMarkdownDossier() {
  ...
  if (!proj.chapters || proj.chapters.length === 0) {
    ...
    proj.chapters.forEach((ch, idx) => {
      ...
      if (ch.scratchpad) md += `\n*Ideias & Lembretes da Cena:* ${ch.scratchpad}\n`;
      if (ch.content) {
        md += `\n#### Texto do Manuscrito:\n\n${ch.content}\n`;
      }
      md += `\n---\n\n`;
